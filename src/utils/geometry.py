...
def cal_normal_distance(pd_proj, gt_proj, reduction=None):
    error = torch.norm(pd_proj-gt_proj, p=2, dim=1, keepdim=True)
    error = error.clamp(0., 1.)
    # error = error.gt(0.3).float()
    # [0, 1]
    # if reduction == 'softmax':
    #     batch_size, num_channels, height, width = error.shape
    #     error = F.softmax(error.reshape(batch_size, num_channels, -1), dim=-1)
    #     error = error.reshape(batch_size, num_channels, height, width)
    return error
